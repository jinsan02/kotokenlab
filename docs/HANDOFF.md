...
최종 갱신 2026-09-17 (**Day 4 완료. 한자 프로브 D1·D2·D3 전부 예측 적중**). 1차 결과 전체는
...
**2차 Day 4 완료. R1·Q7 부정, R5 긍정, 한자 프로브 D1~D3 "T2a = C0".**
**다음은 GPU 가 아니다 — 헤드라인 재작성과 R2·R3 재설계 사전 등록.**
...
한자 프로브까지 끝나 **등록된 GPU 실험은 남아 있지 않다.** 다음은 순서대로:

1. **논문 헤드라인 재작성** — "65% 벽" 은 코사인 스케줄의 산물이었다 (R5).
   본론 2 를 "정체" 가 아니라 "스케줄 조건부 회복" 으로 다시 세운다
2. **"상수 LR 에서도 결국 멈추는가" 를 등록할지** — 168.5MB 에서도 곡선이
   내려가고 있었다. 등록하면 예산 · 판정 경계를 먼저 커밋한다
3. **R2·R3 재설계** — 후보는 행당 노출 · 손상 행 수 · 토큰화 변경.
   `permute` 가 하나를 고립시킨다. 사전 등록 전에는 돌리지 않는다

그 뒤 W4~6 집필 (2페이지 먼저). 마감은 [`VENUES.md`](VENUES.md).
