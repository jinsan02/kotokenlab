...
### 6. Step 6 — 본 CPT 3조건 168.5MB (지금)

`sh scripts/run_phase3_rest.sh`. 세 조건이 같은 원문 168.5MB 를 본다
(Equal-Raw-Data). C0 는 그것을 50M 토큰으로, T2b 는 약 34.7M 토큰으로 처리한다 —
토큰이 적은 것이 손해가 아니라 그게 압축 개선의 실체다.
