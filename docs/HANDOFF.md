...
최종 갱신 2026-09-16 (**Day 3 완료. R5 긍정 — 벽은 코사인 스케줄의 산물**). 1차 결과 전체는
...
**2차 Day 3 완료. R1·Q7 부정, R5 긍정. 다음은 Day 4 = 한자 프로브 D1·D2.**
...
### 다음 — Day 4 = 한자 프로브 D1·D2 (GPU 약 20분)

입력·설정·체크포인트는 2026-09-16 에 준비와 확인을 끝냈다. 명령은
[`SCHEDULE_P2.md` "Day 4"](SCHEDULE_P2.md) 에 그대로 있다.

```
dev_hanja.jsonl   787문서 6.71MB   등록값과 일치
D2 --max-bytes 7000000 은 측정 전에 PLAN.md 에 고정했다
```

D3(KMMLU)는 데이터가 없다. **내일은 등록만** 하고, 다운로드는 확인을 받은 뒤다.

그 뒤 남은 큰 일은 둘이다 — **논문 헤드라인 재작성**, 그리고 R5 가 연
**"상수 LR 에서도 결국 멈추는가"** 를 새로 등록할지 결정.
