...
```
예산 꼬리 회계     완전한 update 경계에서 멈춘다. 원장에 applied/updates 추가   2da591e
채점 바이트 진단   조건 간 분모 차이 351바이트(0.005%). D2 의 n_bytes 재현      04aef2a
분할 감사          9개 split 어디에도 누수 0 (manifest 해시만, 본문 미개봉)     be62085
2주차 동결         PLAN "2주차 run 설정 동결" — 돌리기 전에 커밋                409930a
```

2주차 — 감사의 최소 추가 실험 1번

```
C0 상수 seed 123   1.60h · ok                                                b6aa05a
C0 상수 seed 2026  1.58h · ok                                                b6aa05a
seed 쌍 잔차표     tools/seed_pairs.py 가 적는다                              ca745a6
```
...
- `compare_runs` 가 `pool_extend_docs`·`warmup_bytes` 를 **비교 불가**로 냈다.
  두 필드는 7c3dfda(09-19)에서 생겨 seed 42 의 config 에 없다. 넘기지 않고
  seed 42 이후 학습 경로 커밋 5개를 전부 대조했다 — 실제로 동작을 바꾼 것은
  `2da591e` 하나이고 동결 절에 미리 적혀 있었다. **config 비교는 코드 변경을
  못 잡는다.** 다음에도 커밋 목록을 따로 본다
- `seed_pairs.py` 가 영어·코드에서 R 을 369%·563% 로 냈다. 그 축은 분모
  `B0-Cf` 가 음수라 R 이 정의되지 않는다. 코드가 막고 RULES 14b 에 집행 장치를
  적었다 (`ef03eb0`·`0fde665`)

**남은 교란:** 통제군 update 가 seed 42 는 NA(회계 수정 이전), 새 둘은 1,524 다.
`Cf` 차이에 seed 와 학습량이 섞여 가를 수 없다. 3주차가 이 축을 따로 잰다.

다음은 3주차 — **T2b 상수를 C0 와 같은 1,523 update 로** 3 seed (약 5.3h).
T2b 는 같은 168.5MB 에서 토큰을 적게 내 update 가 1,063 뿐이라, 지금까지의
격차에 "데이터가 같아도 계산량이 달랐다" 가 섞여 있다. 그것을 떼어낸다.
...
재현하는지 확인하고 `--allow code` 를 쓸지, 아니면 비교 대상을 새로 돌린
run 으로만 둘지를 **돌리기 전에** 정해서 PLAN 에 적는다.

### 지난 순서 — 3차 W0 (2026-09-19 완료)
