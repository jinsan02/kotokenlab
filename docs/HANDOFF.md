...
### 다음 — amendment 2주차 (C0 상수 LR seed 123 · 2026)

**2026-09-20 에 1주차(GPU 0시간)를 닫았다.**
...
2주차 동결         PLAN "2주차 run 설정 동결" — 돌리기 전에 커밋                (이 커밋)
```

다음은 `cpt_c0_qwen_r5_seed{123,2026}` 두 개다 (약 3.2h). T2b 상수 3 seed 는
이미 있고 C0 만 seed 42 하나라 대응 쌍이 없다 — 그것만 채운다.
**회계 수정 이후 run 이라 예산 종점이 seed 42 와 한 update 미만 다르다.**
비교표에 세 seed 의 실제 반영량을 함께 싣는다.
