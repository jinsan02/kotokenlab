...
최종 갱신 2026-09-07 (**2차 Day 0 완료**). 1차 결과 전체는
...
2차는 그 65% 회복 정체가 무엇 때문인지 묻는다. **Day 0 에서 도구·사전 등록·
K 보정까지 끝냈고 실험은 하나도 안 돌렸다.** 일정은
[`SCHEDULE_P2.md`](SCHEDULE_P2.md), 설계는 [`SPEC_P2.md`](SPEC_P2.md),
사전 등록은 [`PLAN.md`](PLAN.md).

> Day 0 이 학습 없이 확인한 것: **임베딩 행 40개를 망가뜨리면 토큰 30,000개를
> 치환한 것과 같은 손상(배율 2.04배)이 난다**
> ([`k_calibration.md`](../reports/tables/k_calibration.md)).
> 손상은 몇 행인가가 아니라 어느 행인가로 정해진다.

**아래 표는 1차에서 밟은 순서의 기록이다.** 다음에 무엇을 할지는 10번 절을 본다.
...
Step 4   embedding surgery — E1 부품 평균 채택                  완료
         ────────────────────────────────────────────────
Step 5   노이즈 플로어 조건별 sigma                            완료
폐기     Embedding Alignment — 3라운드 탐침 끝에 뺐다
Step 6   본 CPT 3조건 168.5MB — Q4 부정 · Q5 긍정               완료
Phase 4  등토큰 예산 + N 스윕 — 단순 노출량 가설 미지지            완료
Phase 5  Q6 시스템 벤치마크                                      완료
Phase 6  유효 범위 A(스케일) C(기울기) E(배치)                   완료
         ════════════════════════════════════════════════
         **1차 종료. reports/FINAL_REPORT.md**
금지     phase1-tokenizer-freeze 이후 코퍼스를 바꾸지 않는다
금지     data/final_test 는 아직 열지 않았다
```
