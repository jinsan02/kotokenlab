...
**진단: 새 토큰 30,000개의 중앙값 발화가 168.5MB 전체에서 143회다.** 전체
토큰의 43% 가 새 토큰인데도 그렇다 — 상위 0.4% 가 45,000번씩 먹는다.
896차원 벡터를 143번 업데이트로 밑바닥부터 학습할 수 없다. 초기화가 아니라
노출의 문제다.

산출물: `artifacts/models/cpt_{c0_qwen,t2a_none,t2b_mean}_main_seed42`
(artifacts.tsv 에 등록됨).

### 7. Phase 4 — 등토큰 예산과 N 스윕 (다음)

`sh scripts/run_phase4.sh` (약 3.7시간). 진단이 가리키는 두 축이다.

1. **Equal Token Budget** (스펙 §32~33) — T2b 만 다시 돌린다. C0 와 T2a 는
   이미 정확히 49,922,048 토큰을 봤다. 241,498,793 B 를 준다
2. **N 스윕 N=10,000** — 압축의 83% 를 새 토큰 1/3 로 낸다. 토큰당 노출이
   약 3배가 된다. 예산·풀·시드는 Step 6 과 동일

**예측을 먼저 박아 뒀다** ([`PLAN.md`](PLAN.md) "Phase 4 사전 등록"): 둘 다
격차가 줄되 C0 를 넘지는 못한다. N 스윕이 이를 깨면 결론이 부정에서 조건부
긍정으로 바뀌고, 그때 N=20,000 을 추가해 곡선을 그린다.

**시작 전에 GPU 여유를 확인하라.** Step 6 에서 peak_alloc 11.4GB 에 데스크톱
앱까지 더해져 16.3GB 중 96% 가 찼다. 여유가 550MB 뿐이었다 — 게임·영상
편집을 켜면 OOM 으로 몇 시간이 날아간다.
