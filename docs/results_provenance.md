...
| 1.5B CPT 회복률 | **증거 부족** — 측정하지 않았다 (P3-G 탐침 예정) |
| 1차 절대값 전반 (BPB · 회복률 65%) | **조건부** — 코사인 LR 스케줄에 딸려 있다. 2차 R5 에서 C0 · T2b 모두 상수 LR 이 더 나았다 |
| run 의 `git_dirty` (2026-09-17 이전 행) | **증거 부족** — 커밋 전 원장 행 때문에 거의 모두 1 이다. 해석하지 않는다 (DESIGN_DELTA 3-11) |
| 7B 이상의 손상·회복 | **증거 부족** — 하드웨어가 안 된다. 구조상 `tie_word_embeddings=0` 이라는 것만 `models.tsv` 로 확인됨 |
| 능력 평가 (Level 3) | **부분** — KMMLU 6과목만 (`reports/tables/hanja_probe.md`, record `855c07b`). 0.5B 라 C0 가 찍기보다 약 6.5%p 위로 감도가 낮다. 코퍼스 오염 제거가 구현되지 않아 오염 여부 미확인 (P3 W0-12). T2b 수치는 descriptive |
