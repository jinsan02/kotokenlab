...
- `grad_norm_dmg` · `upd_w_ratio_dmg` 는 **손상·신규 임베딩 행만** 본다
  (P3 W0-5, 2026-09-19 추가). `cpt.py --damaged-rows` 를 준 run 에만 값이 있다.
  전체 임베딩 노름은 "손상된 행이 아직 배우는 중인가" 에 답하지 못한다 —
  151,936행 중 40행이 움직여도 전체 노름은 거의 안 바뀐다.
  `upd_w_ratio_dmg` 는 optimizer step 전후 가중치를 직접 빼서 `||Δw|| / ||w||` 로
  잰다. AdamW 의 이동량은 기울기 크기에 비례하지 않으므로 노름으로 대신할 수 없다
