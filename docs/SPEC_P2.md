...
| **R3** | 손상 **규모** K에 따라 다른가 (간격 재등록 — 아래) | 같다. 1차가 10k·30k 에서 이미 보였다 | 40MB × 3 |
...
행 선택 규칙과 K 보정 절차는 [`PLAN.md`](PLAN.md) "R1~R3 의 행 선택 규칙" 에
등록했다. **보정은 Day 1 전에 Pre-CPT 로만 끝낸다** — 학습 결과를 보고 K 를
고르면 사전 등록이 아니다.
