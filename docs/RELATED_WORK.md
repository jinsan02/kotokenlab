...
> 초기화가 그 간격을 메우는지가 곧 "초기화로 될 일인가" 에 대한 외부 증거가
> 된다. 우리는 노출 부족으로 봤으나 **지지되지 않았다** — 노출을 4.4배로
> 올려도 회복률이 거의 안 움직였다 ([`../reports/tables/phase4.md`](../reports/tables/phase4.md)).
> 병목이 초기화도 노출도 아니라면 ZeTT 의 하이퍼네트워크가 우리보다 나을
> 이유도 약해진다. 그래도 baseline 으로는 여전히 값지다 — 다른 축에서 온
> 초기화가 같은 65% 벽에 걸리는지가 곧 그 벽이 초기화와 무관하다는 증거다.
>
> **2026-09-17 갱신** — "65% 벽" 은 코사인 LR 스케줄의 산물이었다 (2차 R5, 상수 LR
