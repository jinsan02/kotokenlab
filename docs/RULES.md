...
정확한 관계는 `log2(PPL_token) = BPB / (tokens/byte)` 다. 큰 토큰이 언제나
낮은 token PPL 을 만든다는 뜻이 아니라, **예측 단위가 달라 같은 수를 직접
비교할 수 없다는 뜻**이다.
...
## 14b. 회복률(R)의 정의 — 2026-09-19 고정

이 프로젝트에서 **"회복률" 은 아래 하나만 가리킨다.** 외부 연구와 비교할 때
같은 축인지 판단하려면 정의가 한 곳에 있어야 한다.
...
| 기호 | 무엇 | 예 (1차 T2b, 168.5MB 코사인) |
|---|---|---:|
| `B0` | 수술 직후 · **학습 전** 의 한국어 dev BPB | 2.3803 |
| `Bf` | 같은 모델을 CPT 로 예산만큼 돌린 **뒤** 의 같은 지표 | 1.5671 |
| `Cf` | **통제군 C0** (원본 토크나이저, 같은 코퍼스·같은 예산·같은 스케줄) 의 같은 지표 | 1.1375 |

- 지표는 **한국어 dev BPB 하나** 다 (RULES 3번). 영어·코드는 부수 피해로 따로 적고
  R 에 넣지 않는다
- 시점은 **그 run 의 예산 종점** 이다. 곡선 중간 지점의 R 을 말할 때는 예산을
  함께 적는다 (`R(80MB)`). **다른 예산의 체크포인트끼리 비교하지 않는다** —
  2026-09-14 에 R1 이 이것 때문에 판정 불가가 됐다 (`PLAN.md` "R1 판정 기준 정정")
- `Cf` 는 **같은 조건에서 다시 잰 값** 이어야 한다. 스케줄을 바꾸면 통제군도 그
  스케줄로 다시 돌린다 (R5 에서 상수 LR C0 를 따로 돌린 이유)
- 분모는 "수술이 낸 격차", 분자는 "CPT 가 메운 격차" 다. `R = 1` 은 통제군을
  따라잡았다는 뜻이고 `R = 0` 은 하나도 못 메웠다는 뜻이다

**무엇이 이를 강제하는가** — `tools/seed_pairs.py` 는 `B0 - Cf <= 0` 이면 R 을
계산하지 않고 `정의 안 됨` 을 적는다. 영어·코드에서 실제로 분모가 음수다
(C0 조차 한국어 CPT 로 그 축이 나빠진다). 막지 않으면 음수/음수가 영어 369%,
코드 563% 같은 백분율을 만든다 — 2026-09-20 에 도구를 쓰다 실제로 나왔고,
그 자리에서 막았다. R 의 분모는 표에 항상 같이 적는다.

### 2026-09-19 해석 amendment

정의와 과거 사전 등록 판정은 보존하지만, 이후 결과 표의 우선순위는 다음과 같다.

1. `B0`, `Bf`, `Cf` 절대 BPB
2. 최종 잔차 `Bf - Cf`
3. `R` — 등록된 보조·기술 지표

`R` 은 `B0`와 `Cf`에 함께 의존한다. 같은 `Bf-Cf`라도 더 나쁜 `B0`가 더 큰 R을
만들 수 있고, 초기화가 좋아져 `B0`가 낮아지면 R이 작아질 수 있다. 따라서 R의
유사성만으로 **회복 불변성, 공통 메커니즘, 초기화 우열**을 주장하지 않는다.
수술 전 원본 `U0`를 쓸 수 있으면 `D0=B0-U0`, `Df=Bf-Cf`를 함께 낼 수 있으나,
이는 반드시 **사후 기술 분석**으로 표시하고 인과적 손상 분해라고 부르지 않는다.

**외부 연구와 비교할 때.** 이 R 은 *격차를 몇 % 메웠나* 이지 *성능을 몇 % 유지했나*
가 아니다. 유지율(`Bf / Cf` 또는 과제 점수 비)이나 절대 저하폭(`%p`)을 보고한
연구와는 **바로 비교되지 않는다.** 그쪽 수치를 R 로 옮기려면 그 논문이 `B0`(학습
전)과 `Cf`(동일 예산 통제군)를 둘 다 보고해야 한다. 둘 중 하나라도 없으면
"같은 축이 아니다" 라고 적고 넘어간다 — 환산해서 나란히 놓지 않는다.

## 14c. `2σ`의 뜻 — 95% CI가 아니다

이 저장소의 `σ_BPB`는 동일 config의 seed 42/123/2026 final BPB에서 구한
**표본 표준편차(n-1)** 다. paired difference의 표준편차도, 평균의 표준오차도
아니다. `Δ > 2σ`는 2026-08-29에 등록한 **결정 휴리스틱**이며 95% 신뢰구간으로
해석하지 않는다.

- 0과 다른 효과, 실용 경계보다 작은 효과, 통계적 동등성은 서로 다른 주장이다
- 문서·문항 bootstrap은 고정 checkpoint의 표본 불확실성만 나타내며 학습 seed
  불확실성을 대신하지 않는다
- "구별 불가"는 "동등함"이 아니다. 동등성을 말하려면 사전 지정한 margin과
  그 안에 들어오는 적절한 CI가 모두 필요하다
...
| [`DOMAIN_LABELS.md`](DOMAIN_LABELS.md) | 도메인 라벨 신뢰도 — 무엇을 주장할 수 있나 |
