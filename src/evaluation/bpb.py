...
이 프로젝트에서 토크나이저가 다른 모델의 likelihood 를 비교하는 주 지표다.
token PPL 은 예측 단위 자체가 달라 직접 비교할 수 없다. 정확한 관계는
`log2(PPL_token) = BPB / (tokens/byte)` 이므로, 토큰화율이 다르면 같은 BPB도
token PPL 이 달라진다. 분모를 원문 바이트로 고정한 BPB를 쓴다
(docs/RULES.md 3번).
...
세는 방법
    문서를 각각 토큰화해 겹치지 않는 seq_len 토큰 조각으로 자르고, 각 조각에서
    **두 번째 토큰부터** NLL 을 더한다 (첫 토큰은 예측 대상이 아니다). 바이트
    분모도 그 채점 토큰이 표현하는 바이트만 센다. BOS/EOS와 padding은 넣지 않고
    문서 사이 문맥도 잇지 않는다.

    같은 JSONL 문서를 같은 순서로 읽지만 조각 경계는 **토큰 기준**이라 토크나이저
    별로 다르다. 따라서 경계의 첫 토큰으로 빠지는 원문 위치와 최종 `n_bytes`도
    완전히 같지는 않다. 이것은 고정 token-context 배포 조건의 비교다. 동일 원문
    byte-context를 강제한 평가는 별도 질문이며 현재 구현되어 있지 않다. 상류의
    tokenizer 수술은 byte round-trip을 검사하지만, 이 evaluator 자체는 매 run에서
    다시 검사하지 않는다.
