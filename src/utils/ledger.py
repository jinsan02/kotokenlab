...
TRAIN_CURVE_COLUMNS: tuple[str, ...] = (
    ...
    "grad_norm_head",
    # P3 W0-5. "임베딩 기울기가 안 죽는다" 는 관측은 **전체 임베딩** 의 노름이라
    # 손상된 행이 아직 배우는 중인지 말해 주지 않는다. 손상 행만 따로 재고,
    # 노름 대신 **실제로 움직인 양**(AdamW 는 기울기 크기와 비례하지 않는다)을
    # 가중치 크기로 나눠 함께 남긴다. 손상 행 목록을 준 run 에만 값이 있다.
